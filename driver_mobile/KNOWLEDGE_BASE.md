...
- **[SupportHistoryScreen](file:///c:/Users/user/Downloads/Fleet-management-system/driver_mobile/lib/screens/support_history_screen.dart)** - Support History screen displaying Dark Navy header, search bar, filter chips (`All`, `Open`, `In Progress`, `Resolved`, `Rejected`) side-by-side with two action buttons ("Call" and "Message" navigating to their respective manager screens), card list of realistic Indian vehicle maintenance tickets (`Engine Overheating`, `Tyre Puncture`, `Brake Service Required`, `Battery Not Charging`, `GPS Device Not Working`, `Breakdown Assistance`) with priority badges, vehicle registrations, trip IDs, status indicators, and an orange `+` FAB opening RaiseTicketScreen.
