...
export default function LandingFooter() {
  ...
  return (
    <footer className="bg-[#0D1B2A] text-gray-300 pt-12 sm:pt-16 pb-8 px-4 sm:px-6 md:px-8 mt-auto">
      <AnimeStaggerGroup direction="top" className="max-w-[1550px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 md:gap-12 pb-12 border-b border-gray-800">
        ...
        <div className="space-y-4 lg:col-span-1">
          ...
          <div className="flex items-center gap-3 pt-2">
            <a href={platformSettings?.facebookUrl || "#"} target="_blank" rel="noopener noreferrer" className="text-[#A14000] hover:opacity-80 transition-opacity">
            ...
            <a href={platformSettings?.linkedinUrl || "#"} target="_blank" rel="noopener noreferrer" className="text-[#A14000] hover:opacity-80 transition-opacity">
              <span className="sr-only">LinkedIn</span>
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" /></svg>
            </a>
            <a href={platformSettings?.twitterUrl || "#"} target="_blank" rel="noopener noreferrer" className="text-[#A14000] hover:opacity-80 transition-opacity">
              <span className="sr-only">Twitter</span>
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" /></svg>
            </a>
            <a href={platformSettings?.youtubeUrl || "#"} target="_blank" rel="noopener noreferrer" className="text-[#A14000] hover:opacity-80 transition-opacity">
        ...
        <div className="space-y-4 text-xs text-gray-400">
          ...
          <ul className="space-y-3">
            <li className="flex items-center gap-2">
              ...
              <span>{platformSettings?.contactPhone || "+91 1800 200 4567"}</span>
            </li>
            <li className="flex items-center gap-2">
              <svg className="h-4 w-4 text-[#A14000] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              <span>{platformSettings?.contactEmail || "support@fleetmanagement.io"}</span>
            </li>
            <li className="flex items-center gap-2">
              <svg className="h-4 w-4 text-[#A14000] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
              ...
              <span>{platformSettings?.contactAddress || "Bengaluru, Karnataka, India"}</span>
            </li>
            <li className="flex items-center gap-2">
              <svg className="h-4 w-4 text-[#A14000] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
      ...
      <AnimeScrollReveal direction="top" className="max-w-[1550px] mx-auto pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-[10px] text-gray-500 font-medium">
        <div>
