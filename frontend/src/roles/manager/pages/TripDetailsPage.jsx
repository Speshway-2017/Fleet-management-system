    ...
    const fetchTripAndInvoice = async () => {
      try {
        ...
        if (data) {
          ...
          try {
            const tollRes = await managerApi.getTollReceiptsByTripId(data._id);
            const tollData = tollRes.data?.data || tollRes.data;
            if (tollData) {
              setToll(tollData);
            }
          } catch (tollErr) {
            console.error("Failed to load tolls:", tollErr);
          }
        }
      } catch (error) {
